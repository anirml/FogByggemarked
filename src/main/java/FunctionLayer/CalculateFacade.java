...
public class CalculateFacade {
    ...
    public static String polyDraw(Koordinat[] arrayKoor) {
        String polyS = "";
        String koorS ="";
        for (int i = 0; i <arrayKoor.length ; i++) {
            koorS = koorS + (arrayKoor[i].getX()/10+","+arrayKoor[i].getY()/10+" ");
        }
        polyS = "<polygon points = \"" + koorS + "\" style=\"stroke: black; fill: white;\"/>\n";

        return polyS;
    }


    public static String linDraw (int x1, int y1, int x2, int y2) {
        String linie = "";
        linie = "<line x1=\"_x1_\" y1=\"_y1_\" x2=\"_x2_\" y2=\"_y2_\" style=\"stroke: black; fill: white; \"/>\n";
        linie = linie.replace("_x1_", Integer.toString(x1/10));
        linie = linie.replace("_y1_", Integer.toString(y1/10));
        linie = linie.replace("_x2_", Integer.toString(x2/10));
        linie = linie.replace("_y2_", Integer.toString(y2/10));
        return linie;
    }
    ...
    public static String dimVer(int x1, int y1, int x2, int y2, int xText, int yText, int dim) {
        ...
        yDim = "<line x1=\"_x1_\"  y1=\"_y1_\" x2=\"_x2_\"   y2=\"_y2_\" \n" +
                ...
                "marker-start: url(#beginArrow);\n" +
                "marker-end: url(#endArrow);\"/>\n" +
                "<text transform=\"rotate(-90 _xText_,_yText_)\" x=\"_xText_\" y=\"_yText_\">_dim_</text>\n";
...
//******GENERING AF TEGNING **** SLUT ***********************************************************************
    ...
    public static void drawPitch(HttpServletRequest request, int cW, int shedWid, int angle) {

        HttpSession session = request.getSession();
        System.out.println("Er i metode for drawPitch genering");
        ...
        //TopDel af svg
        String topFlatSvg = "<svg version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\"\n" +
                "height=\"80%\" viewBox=\"0 0 _x_ _height_\"\n" +
                "preserveAspectRatio=\"xMinYMin\">\n";
        topFlatSvg = topFlatSvg.replace("_x_", Integer.toString(400 + cW / 10));//
        topFlatSvg = topFlatSvg.replace("_height_", Integer.toString(200 + hmax / 10));//
        ...
        String vBox1FlatSvg = "<svg version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\" " +
                ...
                "width=\"_len_\" height=\"_wid_\" viewBox=\"0 0 _len_ _wid_\"\n" +
                "preserveAspectRatio=\"xMinYMin\">\n";
        vBox1FlatSvg = vBox1FlatSvg.replace("_len_", Integer.toString((cW+2000) / 10));
        vBox1FlatSvg = vBox1FlatSvg.replace("_wid_", Integer.toString(hmax / 10));
        ...
        //remme ender
        remPitchSvg = remPitchSvg+rectDraw(1350,hmax-2100,50,200);
        remPitchSvg = remPitchSvg+rectDraw(1300+cW-600-100,hmax-2100,50,200);
        ...
        String sumFlatSvg = topFlatSvg + vBox1FlatSvg + frameFlatSvg + centerPitchSvg  +
                 ...
                 remPitchSvg + dim1PitchSvg + end0FlatSvg;

        session.setAttribute("svg_drawing1", sumFlatSvg);

    }

    public static void drawFlat(HttpServletRequest request, int cW, int shedWid, int angle) {
